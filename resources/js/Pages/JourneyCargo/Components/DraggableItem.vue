        ...
        <div class="flex flex-col gap-1 w-full">
            <div class="flex flex-row justify-between w-full">
                ...
                <div class="flex flex-col items-start gap-2">
                    Magazzino: {{ props.element.warehouse? props.element.warehouse.name : 'non specificato' }}
                    <div v-if="Number(props.element.warehouse_id) != Number(props.warehouse_id)" class="flex flex-col items-start gap-1">
                        <div class="flex flex-row items-center gap-2">
                            ...
                            <div>
                                ...
                                <select
                                     v-if="is_double_load" 
                                     v-model="download_warehouse_id" 
                                     id="download_warehouse_id" 
            ...
            <div>
                Ord. #<ZeroPaddingId :id=props.element.order.id /> di {{ props.element.order.customer.company_name }}
            </div>
            <div>
...
const download_warehouse_id = ref();
onMounted(() => {
  ...
  download_warehouse_id.value = props.element.pivot?.download_warehouse_id ?? props.warehouse_id ?? null;
});
...
watch(
  ...
  ({ parentWarehouse, elementWarehouse }) => {
    if (Number(parentWarehouse) === Number(elementWarehouse)) {
      ...
      download_warehouse_id.value = null;
...
const manageElementDoubleLoad = () => {
  ...
  emit('double-load-change', props.element.id, is_double_load.value, download_warehouse_id.value);
};
