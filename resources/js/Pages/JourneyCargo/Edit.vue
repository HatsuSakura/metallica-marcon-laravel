  ...
  import ZeroPaddingId from '@/Components/UI/ZeroPaddingId.vue';
      ...
      const user = computed(
        ...
        const JourneyCargoVehicle = computed(() => {
            return props.journeyCargos.find(item => item.cargo_location === 'vehicle') || null;
        });
        const JourneyCargoTrailer = computed(() => {
            return props.journeyCargos.find(item => item.cargo_location === 'trailer') || null;
        });
      ...
      const form = useForm({
        ...
        is_grounding_truck: Boolean(JourneyCargoVehicle.value.is_grounded),
        ...
        is_grounding_trailer: Boolean(JourneyCargoTrailer.value.is_grounded),
    ...
    const normalizeForPayload = (arr, cargoWarehouseId, otherWarehouseId) => {
      return (arr || []).map(el => {
        ...
        let wd = el?.pivot?.download_warehouse_id ?? null;
      ...
      const update = () => {
        ...
        form.items_fulfill = (listRiempimento.value || []).map(x => ({
          id: x.id, is_double_load: 0, download_warehouse_id: truckWh // o trailerWh: decidi tu
        }));
      ...
      const distributeOrderItems = () => {
        ...
        const JourneyCargoVehicle = props.journeyCargos.find(item => item.cargo_location === 'vehicle') || null;
        const JourneyCargoTrailer = props.journeyCargos.find(item => item.cargo_location === 'trailer') || null;
...
VECCHIA LOGICA
      ...
      const manageElementDoubleLoad = (element_id, is_double_load, download_warehouse_id) => {
        ...
        element.pivot.download_warehouse_id = download_warehouse_id ?? null;
      };
  ...
  function ensurePivot(obj) {
    ...
    if (typeof obj.pivot.download_warehouse_id === 'undefined') obj.pivot.download_warehouse_id = null;
    return obj;
  }
