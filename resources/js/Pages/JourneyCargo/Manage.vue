...
<div class="w-full">
    ...
    <div class="mb-4 flex flex-row justify-between items-center w-full">
          <div class="text-lg font-medium">
              Gestione Materiali Scaricati in {{ journeyCargo.warehouse.name }} da cassone {{ journeyCargo.cargo_location === 'vehicle'? 'Motrice': 'Rimorchio' }}
          </div>
          <div>
        ...
        <Box class="flex flex-row mb-2 w-full">
          ...
          <div v-if="form.has_crane" class="w-1/2 flex flex-row justify-between items-center">
            ...
            <div class="flex flex-row items-center gap-2 justify-start">
              ...
              <select id="crane_operator_user_id" v-model="form.crane_operator_user_id" class="select select-bordered">
                ...
                <option v-for="worker in warehouseWorkers.filter(worker => worker.is_crane_operator)" :key="worker.id" :value="worker.id">
        ...
        <div>
            <WorkerItemRow
                ...
                :parentMachineryTime="Number(form.machinery_time_minutes)"
            ...
            <div v-if="otherJourneyCargo.items.length > 0">
                <div class="flex flex-row justify-between items-center w-full mb-2">
                    <div class="text-lg font-medium">
                        ...
                        {{ otherJourneyCargo.warehouse.name }}
                        da cassone 
                        {{ otherJourneyCargo.cargo_location === 'vehicle'? 'Motrice': 'Rimorchio' }}
                    </div>
                    <div>
...
const form = useForm({
    has_crane: props.journeyCargoItems.has_crane || false,
    crane_operator_user_id: props.journeyCargoItems.crane_operator_user_id || '',
    machinery_time_minutes: props.journeyCargoItems.machinery_time_minutes || 0,
...
/**
 * Calcolo del machinery_time_minutes
 */
    // Computed property for hours with getter and setter
    const machinery_time_hh = computed({
      get() {
        return Math.floor(form.machinery_time_minutes / 60);
      },
      set(newHour) {
        // Update machinery_time_minutes based on the new hours and current minutes
        form.machinery_time_minutes = Number(newHour) * 60 + Number(machinery_time_mm.value);
    ...
    const machinery_time_mm = computed({
      get() {
        return form.machinery_time_minutes % 60;
      },
      set(newMin) {
        // Update machinery_time_minutes based on the current hours and new minutes
        form.machinery_time_minutes = Number(machinery_time_hh.value) * 60 + Number(newMin);
        updateMachineryItemFraction();
      }
    });
...
const nonManualCount = computed(() => {
  if (props.journeyCargoItems.items){
    return props.journeyCargoItems.items.filter(item => manualFractions[item.id] === undefined && item.is_crane_eligible).length
  }
  return 0
...
const remainingTime = computed(() => {
  return form.machinery_time_minutes - totalManual.value
})
...
const saveJourneyCargo = async(journeyCargo_id) => {
      try {
          ...
          const response = await axios.put(url, {
            ...
            crane_operator_user_id : form.crane_operator_user_id,
            machinery_time_minutes : form.machinery_time_minutes,
          });
