          ...
          <div class="flex flex-row gap-2 w-full mt-4">
            <div class="flex-1">
              <div class="w-full flex flex-col gap-2 mb-4">
                ...
                <div class="flex flex-row justify-center items-center gap-2">
                  ...
                  <div class="flex flex-col gap-1">
                    <select v-if="isDownloadableTruck" v-model="form.warehouse_id_truck" id="warehouse_id_truck" class="select select-bordered">
                  ...
                  <!-- ORDINE di SCARICO -->
                  <div class="flex flex-col gap-1">
                    <select v-if="doubleLoad" v-model="computedOrderTruck" id="download_sequence_truck" class="select select-bordered">
            ...
            <div class="flex-1">
              <div class="flex flex-col gap-2 mb-4">
                <div v-if="props.journey.trailer" class="flex flex-row justify-center items-center gap-1">
                ...
                <div class="flex flex-row justify-center items-center gap-2">
                  ...
                  <!-- MAGAZZINO Select/Option -->
                  <select v-if="isDownloadableTrailer" v-model="form.warehouse_id_trailer" id="warehouse_id_trailer" class="select select-bordered">
                    <option value="" disabled>Seleziona Magazzino</option>
                    <option v-for="warehouse in props.warehouses" :key="warehouse.id" :value="warehouse.id">
                    {{ warehouse.name }}
                    </option>
                  ...
                  <!-- ORDINE di SCARICO -->
                  <select v-if="doubleLoad" v-model="computedOrderTrailer" id="download_sequence_trailer" class="select select-bordered">
                        <option value="" disabled>Ordine</option>
                        <option key="1" value=1>1°</option>
                        <option v-if="props.journey.trailer_id" key="2" value=2>2°</option>
                  </select>
        ...
        <div class="col-span-3">
          <Box>
            ...
            <div class="flex flex-row justify-between items-center gap-2 mb-1">
              ...
              Partenza: {{ dayjs(props.journey.planned_start_at).format('YYYY-MM-DD HH:mm') }}
            </div>

            <div class="flex flex-row justify-between items-center gap-2 mb-1">
              <font-awesome-icon :icon="['fas', 'person-walking-arrow-loop-left']" class="text-2xl"/>  
              Rientro previsto: {{ props.journey.planned_end_at? dayjs(props.journey.planned_end_at).format('YYYY-MM-DD HH:mm') : 'non valorizzato' }}
            </div>
          ...
          <div class="sticky top-24 shadow-md px-4 pb-4 mt-4 rounded-md flex flex-col gap-4 w-full">
            <div class="flex flex-col">
              ...
              <div v-for="order in props.orders">
                {{order.id}} {{ order.customer.company_name }} {{ order.customer.id }} {{ order.index }}
              </div>
            </div>
...
import ZeroPaddingId from '@/Components/UI/ZeroPaddingId.vue';
    ...
    const form = useForm({
      ...
      warehouse_id_truck: '',
      is_grounding_truck: false,
      download_sequence_truck: '',
      warehouse_id_trailer: '',
      is_grounding_trailer: false,
    ...
    // Evaluate if it is a double_load case and if the "order" is needed
    const doubleLoad = computed(() => {
    ...
    const create = () => {
      ...
      form.post(route('journeyCargo.store'));
    }
    ...
    onMounted(() => {
      distributeOrderItems();
          ...
          const manageElementDoubleLoad = (element_id, is_double_load, download_warehouse_id) => {
        console.log('manageElementDoubleLoad PARAMS', element_id, is_double_load, download_warehouse_id);
        ...
        element.pivot.download_warehouse_id = download_warehouse_id;
      };
