        ...
        <div class="accordion-custom-item border-b" data-id="section-2">
          ...
          <div class="accordion-custom-body p-4 bg-gray-100">
            ...
            <div class="">
              ...
              <VueDatePicker v-model="form.withdrawn_at"  class="input input-bordered" @closed="manageDate"></VueDatePicker>
              <div class="input-error" v-if="form.errors.withdrawn_at">
                {{ form.errors.withdrawn_at }}
              </div>
            </div>
    ...
    <div class="card bg-base-100 w-full shadow-2xl p-4">
      ...
      <div class="">
        <label class="label">Percentuale Residua
          <div class="radial-progress" :style="{ '--value': form.residue_percentage }" role="progressbar">{{ form.residue_percentage }}%</div>
        </label>
        <input v-model.number="form.residue_percentage" type="range" min="0" max="100" step="5"
          class="w-full h-4 bg-gray-200 dark:bg-gray-700 runded-lg appearence-none cursor-pointer" />
        <div class="input-error" v-if="form.errors.residue_percentage">
          {{ form.errors.residue_percentage }}
        </div>
      </div>
      ...
      <div class="">
        <label class="label" for="is_manual_entry">Inserimento Manuale</label>
        <input v-model="form.is_manual_entry" id="is_manual_entry" type="checkbox" class="toggle" checked="checked" />
      </div>
...
const form = useForm({
  ...
  created_by_user_id: user ? user.id : null, // Fallback to null if user is not defined
  vehicle_id: null,
  trailer_id: null,
  driver_id: null,
...
watch(user, (newUser) => {
  if (newUser) {
    form.created_by_user_id = newUser.id;
  }
}, { immediate: true });

const create = () => {
  // Before submitting, format the date correctly
  form.withdrawn_at = dayjs(form.withdrawn_at).format('YYYY-MM-DD HH:mm:ss');
...
const manageDate = (date) => {
  ...
  if(form.withdrawn_at){
    console.log('data inserita', form.withdrawn_at)
    console.log('giorno della settimana', form.withdrawn_at.getDay())
    const hours = currentSite.value.timetable.hours_json
    console.log('array Orari', hours)

    const daySchedule = JSON.parse(hours).find(
        (item) => item.position === form.withdrawn_at.getDay()
    );
      if (daySchedule) {
