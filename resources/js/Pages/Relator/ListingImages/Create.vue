    ...
    <Box v-if="listing.images.length" class="mt-4">
        ...
        <section class="mt-4 grid grid-cols-3 gap-4">
            <div v-for="image in listing.images" :key="image.id" class="flex flex-col justify-between">
                ...
                <Link 
                    :href="route('relator.listing.image.destroy', {listing: props.listing.id, image: image.id} )"
                    method="delete"
                    as="button"
...
Inertia.on('progress', (event) => {
    if (event.detail.progress.percentage){
        //NProgress.set.apply((event.detail.progress.percentage/100) * 0.9)
    }
})
...
const canUpload = computed(() => form.images.length) // se ho immagini, length != 0

const upload = () => {
    form.post(
        route('relator.listing.image.store', {listing: props.listing.id}),
        {
